...
const FileUpload = ({ setTranscription, loading, setLoading }) => {
  ...
  const handleFileChange = (selectedFile) => {
    if (selectedFile) {
      ...
      const isAllowedByName = /\.(wav|mp3|webm|ogg|m4a|mp4)$/i.test(
        selectedFile.name,
      );

      if (allowed.includes(selectedFile.type) || isAllowedByName) {
        setFile(selectedFile);
        setError("");
      } else {
        setError("Please upload a WAV, MP3, WEBM, OGG, or MP4 file");
        setFile(null);
      }
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE_URL}/api/transcribe/upload`,
        formData,
        {
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      >
        <input
          ref={fileInputRef}
          type="file"
          accept=".wav,.mp3,.webm,.ogg,.mp4,audio/wav,audio/mpeg,audio/webm,audio/ogg,audio/mp4"
